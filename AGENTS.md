...
**Ptolomeu** — macOS menu bar desktop app: a global command palette (⌘+Shift+Space) with a plugin-based search architecture and an embedded Codex chat window. Built with Electrobun (Bun runtime, not Electron), React 19, Tailwind CSS 4, and Vite. Uses native Objective-C for window overlay and global hotkey registration.
...
3. **Chat renderer** (`src/chatview/`) — separate React app for the Codex chat surface. Vite config is `vite.chat.config.ts`, root `src/chatview/`, output `dist-chat/`, copied to `views/chatview/`. Two webviews run in the same Bun process but with isolated transports.
...
- `Codex` — Codex session list + create-new (opens the chat window)

Registry in `registry.ts` (`PLUGIN_REGISTRY`, `PLUGIN_META`); active plugin is managed by `ProviderContextProvider`.

### Settings

`src/mainview/settings/` — dialog with three tabs: **Plugins** (per-plugin enable + per-plugin config nested as sub-items, e.g. GitHub token, Codex auth, MCP servers), **Rede** (proxy mode + manual proxy with Keychain-backed password), **Geral** (analytics consent, etc.). State lives in `SettingsContext` with debounced RPC persistence to `~/Library/Application Support/com.ptolomeu.app/settings.json`. Plugin reordering and custom GitHub filters use `@dnd-kit` drag-drop. Main-process IO in `src/bun/settings.ts` + `settings-io.ts`.
...
- Codex session: `claudeListSessions`, `claudeCreateSession`, `claudeResumeSession`, `claudeSendMessage`, `claudeStopGeneration`, `claudeDeleteSession`, `claudeGetSessionMessages`, `claudeOpenChat`
- Codex auth: `claudeGetAuthStatus`, `claudeLoginSSO`, `claudeLogoutSSO`, `claudeSetBedrock`, `claudeGetBedrock`
...
- `setWindowShowCallback(JSCallback)` — direct callback when the hotkey transitions the window from hidden to visible (bypasses the unreliable delegate chain so the bun side can refresh Codex session list)
...
### Codex integration (`src/bun/Codex/`)

Built on `@anthropic-ai/Codex-agent-sdk` (`unstable_v2_*` APIs).
...
- Workspace isolation is a security boundary, not a hint — when adding tools to the Codex integration, make sure paths stay inside the conversation's project directory
- Settings schema tolerates unknown keys in the `Codex` block — the validator in `src/bun/settings.ts` reads only known fields and re-serializes without extras. Retiring a setting is safe: users on older `settings.json` files won't hit validation errors, and their obsolete keys drop on the next save.
